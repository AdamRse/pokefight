...
class FightsManager{
    ...
    public function fightTurnBased(Hero $hero , Monster $monster){
        ...
        if($monster->getHp() > 0 && $hero->getHp() > 0 ){
            $logCombat[] = "<span class='logNewRound'>Round <b>$this->_round</b></span>";
            $logCombat[] = $hero->hit($monster);
            if($monster->getHp() > 0)
                $logCombat[] = $monster->hit($hero);
        }
        ...
        $monster->saveSession();
        $this->_heroManager->save($hero);
        return array("status" => 1, "logs" => $logCombat, "heroHp" => $hero->getHp(), "monsterHp" => $monster->getHp());//Status : 0=combat en cours, 1=win, 2=lose
    }
